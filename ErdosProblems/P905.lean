...
lemma degree_add_degree_le_card_add_commonNeighbors
  ...
  refine' le_trans _ (Finset.sum_le_sum fun x _ => ?_)
  show (1 + if G.Adj u x ∧ G.Adj v x then 1 else 0) ≥
    (if G.Adj u x then 1 else 0) + (if G.Adj v x then 1 else 0)
  · aesop
...
lemma bollobas_nikiforov_of_card_le
      ...
      calc
        ∑ u ∈ Finset.univ, ∑ v ∈ G.neighborFinset u,
            ...
            = (Fintype.card V + maxTriangleDegree G) * ∑ u : V, G.degree u := by
              simp [SimpleGraph.degree, SimpleGraph.neighborFinset, Finset.mul_sum, mul_comm]
  ...
  have h_handshake :
      ∑ x, ∑ x ∈ G.neighborFinset x, G.degree x = ∑ x, G.degree x * G.degree x := by
...
lemma sum_triangleDegree_eq_three_mul_cliqueFinset [DecidableEq V]
  ...
  rotate_left
  use fun e =>
    ∑ s ∈ Finset.filter (fun s => G.IsNClique 3 s) Finset.univ,
      if e ∈ Finset.image (fun p : V × V => Sym2.mk p) (Finset.offDiag s) then 1 else 0
  · rcases e with ⟨u, v⟩
    simp +decide [SimpleGraph.commonNeighbors]
    refine Finset.card_bij (fun w _hw => {u, v, w}) ?_ ?_ ?_ <;> simp_all +decide [SimpleGraph.isNClique_iff]
    · intro w hu hv
    ...
    have exists_concordant_edge :
      ...
      obtain ⟨e, he⟩ :
          ∃ e ∈ ({s(u, v), s(u, w), s(v, w)} : Finset (Sym2 V)),
            (y ∈ G.commonNeighbors e.out.1 e.out.2 ∨
              (¬G.Adj y e.out.1 ∧ ¬G.Adj y e.out.2)) := by
        by_cases hyu : G.Adj y u <;> by_cases hyv : G.Adj y v <;> by_cases hyw : G.Adj y w <;>
          simp +decide [*, SimpleGraph.commonNeighbors]
        all_goals
          simp_all +decide [SimpleGraph.adj_comm, Quot.out]
        all_goals
          have := Classical.choose_spec (show ∃ x : V × V, s(x.1, x.2) = s(u, v) from ⟨⟨u, v⟩, rfl⟩)
          have := Classical.choose_spec (show ∃ x : V × V, s(x.1, x.2) = s(u, w) from ⟨⟨u, w⟩, rfl⟩)
          have := Classical.choose_spec (show ∃ x : V × V, s(x.1, x.2) = s(v, w) from ⟨⟨v, w⟩, rfl⟩)
...
theorem bollobas_nikiforov_of_card_gt
  ...
  have h_beta_large_enough : (n : ℤ) ≤ 6 * β := by
    ...
    push_neg at h_beta_large_enough
